...
//이름이 있는 지역내부 클래스. 실행흐름 중에 클래스 로딩이 일어난다.  
		...
		}//class MyActionListener3->실행문
		//btn.addActionListener(new MyActionListener3());
		
	
		//익명의(이름이 없는)지역 내부 클래스.
		ActionListener l = new ActionListener() {//객체 값을 왼쪽에 대입함
		...
		};//세미콜론이 있는것은 대입문(대입연산자)이기 떄문에 있는것이다.
 //btn.addActionListener(l);
		...
		*/
		...
		// 	인터페이스가 추상메소드 1개만 가졌을 경우만 람다식을 사용 가능하다.
		
		//btn.addActionListener(event->System.out.println("버튼 클릭됨"));//매개변수가 없을때는 (), 매개변수가 1개이상일때 람다식에서()생략가능
																	//실행문 1개일때 중괄호 생략가능 ; 도 없어야 한다.
		btn.addActionListener(event->{	String str = ta.getText();//외부 클래스 ta접근
		ta.setText(str+"버튼 클릭됨\n");
	...
	private void setupView() {//화면 모양 설정
		...
		Container c=getContentPane();//지역변수
